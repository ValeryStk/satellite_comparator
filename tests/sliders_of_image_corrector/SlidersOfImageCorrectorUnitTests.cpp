...
void SlidersOfImageCorrectorUnitTests::helper_for_test_coef(
    const int slider_value, const double expected_multiplier_value,
    const QString &name_test, SlidersOfImageCorrector *sic,
...
void SlidersOfImageCorrectorUnitTests::testSliderImageCorrector() {
    ...
    QVERIFY2(sic->getCoefLight() == NEUTRAL_MULTIPLIER,
             "Initial value for light slider");
    QVERIFY2(sic->getCoefSaturation() == NEUTRAL_MULTIPLIER,
             "Initial value for saturation slider");
    QVERIFY2(slider_saturation->maximum() == SLIDER_MAX_VALUE,
             "MAX value for saturation slider");
    QVERIFY2(slider_light->maximum() == SLIDER_MAX_VALUE,
             "MAX value for light slider");
    QVERIFY2(slider_saturation->minimum() == SLIDER_MIN_VALUE,
             "MIN value for saturation slider");
    QVERIFY2(slider_light->minimum() == SLIDER_MIN_VALUE,
             "MIN value for light slider");
    ...
    QVector<test_params> params = {
        {SLIDER_MAX_VALUE, MAX_MULTIPLIER, wrong_message + "MAX MULTIPLIER"},
        {SLIDER_INITIAL_VALUE, NEUTRAL_MULTIPLIER,
         wrong_message + "INITIAL MULTIPLIER"},
        {SLIDER_MIN_VALUE, MIN_MULTIPLIER, wrong_message + "MIN MULTIPLIER"},
        {SLIDER_MAX_VALUE * 0.75, 4.5, wrong_message + "0.75"},
        {SLIDER_MAX_VALUE * 0.25, 0.625, wrong_message + "0.25"}};

    for (int i = 0; i < params.size(); ++i) {
        auto p = params[i];
        helper_for_test_coef(p.slider_value, p.expected_multiplier,
                             p.error_message, sic.get(), width);
    }
}
