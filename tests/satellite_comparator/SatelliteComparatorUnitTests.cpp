...
void SatelliteComparatorUnitTests::testSatelliteComparatorBaseCheck() {
    ...
    QVERIFY2(
        BASE_CHECK_RESULT::SIZES_ARE_NOT_THE_SAME ==
    ...
    QVERIFY2(
        ...
        "Sizes are not the same");
    QVERIFY2(
        BASE_CHECK_RESULT::WAVES_IS_NOT_SORTED ==
            sc->base_check_before_interpolation({10, 9, 8, 7}, {1, 2, 3, 4}),
        "Check not sorted wave1 result");
}

void SatelliteComparatorUnitTests::
    testSatelliteComparatorLinearInterpolation() {
    auto sc = std::make_unique<SatteliteComparator>();
    BASE_CHECK_RESULT status;
    auto result = sc->interpolate({1, 2, 3, 4, 5}, {1.5, 2.6, 3.7, 4.8, 5.6},
                                  {1.2, 2.2, 3.4, 4.5, 4.6}, status);
...
void SatelliteComparatorUnitTests::testLoadSatelliteDataJson() {
    ...
    // qDebug()<<json_object;
    QVector<double> common_waves = jsn::getVectorDoubleFromJsonArray(
        json_object.value("_common_wave_grid").toArray());
    QJsonObject satellites = json_object.value("satellites").toObject();
    QJsonArray responses =
        satellites.value("landsat8").toObject().value("responses").toArray();
