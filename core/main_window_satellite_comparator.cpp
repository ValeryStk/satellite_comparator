...
MainWindowSatelliteComparator::MainWindowSatelliteComparator(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindowSatelliteComparator),
      ...
      m_is_image_created(false), m_is_bekas(false),
      m_scene_text_item_metric_value(new QGraphicsTextItem),
      bekas_window(nullptr), time_row_indexes_plot(new QCustomPlot)

{
  ...
  addBaseItemsToScene();
  setUpPreviewPlot();
  connect(ui->graphicsView_satellite_image, SIGNAL(pointChanged(QPointF)), this,
          SLOT(cursorPointOnSceneChangedEvent(QPointF)));
...
void MainWindowSatelliteComparator::openBekasSpectraData() {
  ...
  connect(bekas_window,
          SIGNAL(sendSampleForSatelliteComparator(QVector<double>,
                                                  QVector<double>)),
          this,
  ...
  if (m_satelite_type == sad::SATELLITE_TYPE::LANDSAT_8 ||
    ...
    if (m_is_bekas) {
      ...
      size_t elems_to_copy =
          std::min(static_cast<size_t>(data.size()), static_cast<size_t>(5));
  ...
  if (extension == "json") {
    ...
    if (jo.contains("LANDSAT_METADATA_FILE")) {
      QJsonObject image_attributes =
          jsn::getValueByPath(jo, {"LANDSAT_METADATA_FILE", "IMAGE_ATTRIBUTES"})
              .toObject();
      ...
      QJsonValue radiance_value =
          jsn::getValueByPath(jo, {"LANDSAT_METADATA_FILE",
      ...
      m_geo.ulX =
          projection["CORNER_UL_PROJECTION_X_PRODUCT"].toString().toDouble();
      m_geo.ulY =
          projection["CORNER_UL_PROJECTION_Y_PRODUCT"].toString().toDouble();
      m_geo.resX =
      ...
      for (int i = 0; i < LANDSAT_BANDS_NUMBER; ++i) {
        ...
        if (i < 7) { // TODO TEST Есть только до 9 канала TEST
          ...
          double mult_refl = radiance[sad::landsat9_mult_reflectence_keys[i]]
                                 .toString()
                                 .toDouble();
          double add_refl = radiance[sad::landsat9_add_reflectence_keys[i]]
                                .toString()
                                .toDouble();
    ...
    auto file_names =
        getLandSat9BandsFromTxtFormat(headerName, landsat_gui_available_bands);
    ...
    for (int i = 0; i < LANDSAT_BANDS_NUMBER; ++i) {
      if (data.landsat9_missed_channels[i])
        continue;
      file_names.append(data.product_contents.file_name_bands[i]);
      landsat_gui_available_bands.append(sad::landsat_bands_gui_names[i]);
      m_reflectance_mult_add_arrays[i][0] =
          data.level2_surface_reflectance_parameters.reflectance_mult_band[i]
              .toDouble();
      m_reflectance_mult_add_arrays[i][1] =
          data.level2_surface_reflectance_parameters.reflectance_add_band[i]
              .toDouble();
    }
    read_landsat_bands_data(file_names);
    ...
    m_geo.resY = -m_geo.resX;
    m_geo.ulX =
        data.projection_attributes.corner_ul_projection_x_product.toDouble();
    m_geo.ulY =
        data.projection_attributes.corner_ul_projection_y_product.toDouble();
...
void MainWindowSatelliteComparator::processBekasDataForComparing(
  ...
  if (m_satelite_type == sad::UNKNOWN_SATELLITE)
    return;
...
void MainWindowSatelliteComparator::updateImage() {
  if (m_image_item) {
    ...
    if (coef_light == 1 && coef_saturation == 1)
      return;
    QImage imgNew =
...
QStringList MainWindowSatelliteComparator::getLandSat9BandsFromTxtFormat(
  ...
  while (ts.readLineInto(&temp)) {
    ...
    if (isGroupProductContentsStart == false)
      continue;
...
QString MainWindowSatelliteComparator::getLandSatSpaceCraftIDFromTxtFormat(
  ...
  while (ts.readLineInto(&temp)) {
    ...
    if (isGroupImageAttributes == false)
      continue;
...
void MainWindowSatelliteComparator::clearLandsat9DataBands() {
  for (int i = 0; i < LANDSAT_BANDS_NUMBER; ++i) {
    if (m_landsat9_missed_channels[i])
      continue;
    if (m_landsat9_data_bands[i] == nullptr)
      continue;
...
void MainWindowSatelliteComparator::cursorPointOnSceneChangedEventTimeRow(
  ...
  if (pos.x() >= xSize || pos.x() < 0)
    return;
  if (pos.y() >= ySize || pos.y() < 0)
    return;
  ...
  for (int i = 0; i < m_time_row.size(); ++i) {
    ...
    if (m_points[i].x() >= xSize || m_points[i].x() < 0)
      return;
    if (m_points[i].y() >= ySize || m_points[i].y() < 0)
      return;
  }
  ...
  for (int i = 0; i < m_points.size(); ++i) {
    ...
    for (int j = 0; j < m_time_row[i].size(); ++j) {
      ...
      if (one_ksy_value == 0 || one_ksy_value > 1)
        continue;
      if (j == red_band_index) {
...
MainWindowSatelliteComparator::getLandsat8Ksy(const int x, const int y) {
  if (m_is_image_created == false)
    return {};
  int xSize = m_landsat9_bands_image_sizes->first;
  int ySize = m_landsat9_bands_image_sizes->second;
...
void MainWindowSatelliteComparator::paintSamplePoints(const QColor &color) {
  ...
  std::thread t1(&MainWindowSatelliteComparator::processLayer, this, new_layer,
                 xSize, 0, midY, sample, color, offset1);
  std::thread t2(&MainWindowSatelliteComparator::processLayer, this, new_layer,
                 xSize, midY, ySize, sample, color, offset2);
  t1.join();
  t2.join();
...
inline double
MainWindowSatelliteComparator::euclideanDistance(const QVector<double> &v1,
    ...
    for (int x = 0; x < nXSize; ++x) {
      ...
      for (int j = 0; j < bands.size(); ++j) {
        ...
        if (bands[j].second == BLUE) {
          B = static_cast<int>(
              m_landsat9_data_bands[bands[j].first][y * nXSize + x] / 255.0);
          choosedColor = BLUE;
  ...
  for (int y = 0; y < nYSize; ++y) {
    for (int x = 0; x < nXSize; ++x) {
      ...
      for (int j = 0; j < bands.size(); ++j) {
        int choosedColor = -1;
        if (bands[j].second == BLUE) {
          B = static_cast<int>(band_data[bands[j].first].data[y * nXSize + x] /
                               255.0);
          choosedColor = BLUE;
        } else if (bands[j].second == GREEN) {
          G = static_cast<int>(band_data[bands[j].first].data[y * nXSize + x] /
                               255.0);
          choosedColor = GREEN;
        } else if (bands[j].second == RED) {
          R = static_cast<int>(band_data[bands[j].first].data[y * nXSize + x] /
                               255.0);
          choosedColor = RED;
        }
        if (bands.size() == 1) {
...
void MainWindowSatelliteComparator::processLayer(uchar *layer, int xSize,
  ...
  for (int y = yStart; y < yEnd; ++y) {
    for (int x = 0; x < xSize; ++x) {
      ...
      if (m_is_bekas) {
        size_t elems_to_copy =
            std::min(static_cast<size_t>(ksy.size()),
...
void MainWindowSatelliteComparator::setUpToolWidget() {
  ...
  connect(m_layer_roi_list, SIGNAL(createTimeRowGradient(const QString)), this,
          SLOT(calculate_time_row_gradient(const QString)));
...
void MainWindowSatelliteComparator::clear_all_layers() {
  if (m_layers_search_result_items.empty())
    return;
...
QString
MainWindowSatelliteComparator::getDateTimeFromXML(const QString &xmlFilePath) {
  QFile file(xmlFilePath);
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    qWarning() << "Не удалось открыть файл:" << xmlFilePath;
    return "";
...
MainWindowSatelliteComparator::getDataFromJsonForLandsat8_9_TimeRow(
  ...
  if (extension == "json") {
    if (jo.contains("LANDSAT_METADATA_FILE")) {
      ...
      QString timeStr =
          image_attributes.value("SCENE_CENTER_TIME").toString().remove('Z');
      ...
      landsat_metadata.projection_attributes.corner_ul_projection_x_product =
          projection["CORNER_UL_PROJECTION_X_PRODUCT"].toString().toDouble();
      landsat_metadata.projection_attributes.corner_ul_projection_y_product =
          projection["CORNER_UL_PROJECTION_Y_PRODUCT"].toString().toDouble();
      ...
      for (int i = 0; i < LANDSAT_BANDS_NUMBER; ++i) {
        auto sorted_order_index =
            sad::sorted_landsat_bands_order_by_wavelength[i];
        if (check_bands.value(sad::landsat9_bands_keys[sorted_order_index])
                .isUndefined()) {
          continue;
        }
        sad::BAND_DATA bd;
        ...
        bd.height = yS;
        double mult_refl =
            radiance[sad::landsat9_mult_reflectence_keys[sorted_order_index]]
                .toString()
                .toDouble();
        double add_refl =
            radiance[sad::landsat9_add_reflectence_keys[sorted_order_index]]
                .toString()
                .toDouble();
...
QVector<QImage> MainWindowSatelliteComparator::get_cropedImages_for_time_row(
  ...
  for (int i = 0; i < m_time_row.size(); ++i) {
    ...
    QImage img =
        QImage(data.get(), nXSize, nYSize, nXSize * 3, QImage::Format_RGB888)
            .copy();
...
void MainWindowSatelliteComparator::showTimeRowIndexesDataViaPlot(
  ...
  if (time_row_indexes_plot->graphCount() == 0) {
    ...
    qcpg_ndvi->setScatterStyle(QCPScatterStyle(
        QCPScatterStyle::ssCircle, QPen(ndvi_color), QBrush(ndvi_color), 10));
    qcpg_ndwi->setScatterStyle(QCPScatterStyle(
        QCPScatterStyle::ssCircle, QPen(ndwi_color), QBrush(ndwi_color), 10));
  ...
  if (time_row_indexes_plot->isHidden())
    time_row_indexes_plot->show();
}

bool MainWindowSatelliteComparator::isDataCloudShadow_OK(
    QVector<QPointF> &points) {
  if (m_time_row_qa_mask.empty())
    return false;
  if (points.empty())
    return false;
  ...
  for (int i = 0; i < points.size(); ++i) {
    auto index =
        ((int)points[i].y() * m_time_row_qa_mask[i].width) + (int)points[i].x();
...
sad::NDWI_NDVI_TIME_ROW MainWindowSatelliteComparator::getIndexesForTimeRow(
  ...
  for (int i = 0; i < points.size(); ++i) {
    ...
    for (int j = 0; j < m_time_row[i].size(); ++j) {
      double value =
          m_time_row[i][j].data[((int)points[i].y() * m_time_row[i][j].width) +
                                (int)points[i].x()];
...
double
MainWindowSatelliteComparator::calculate_slope(const QVector<double> &values) {
