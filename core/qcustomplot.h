...
// Qt version < 6.2.0: to get metatypes Q_GADGET/Q_ENUMS/Q_FLAGS in namespace we
// have to make it look like a class during moc-run
#if QT_VERSION >= 0x060200  // don't use QT_VERSION_CHECK here, some moc
                            // versions don't understand it
namespace QCP {
Q_NAMESPACE  // this is how to add the staticMetaObject to namespaces in newer
             // Qt versions
...
public:
    ...
    /*!
      Defines the different units in which the image resolution can be specified
      in the export functions.
    ...
    */
    enum ResolutionUnit {
        ...
        ,
        ruDotsPerCentimeter  ///< Resolution is given in dots per centimeter
                             ///< (dpcm)
        ,
...
*/
enum ExportPen {
    epNoCosmetic  ///< Cosmetic pens are converted to pens with pixel width 1
                  ///< when exporting
    ,
...
enum AntialiasedElement {
    ...
    ,
    aeFills = 0x0100  ///< <tt>0x0100</tt> Borders of fills (e.g. under or
                      ///< between graphs)
    ,
    aeZeroLine = 0x0200  ///< <tt>0x0200</tt> Zero-lines, see \ref
                         ///< QCPGrid::setZeroLinePen
    ,
    aeOther = 0x8000  ///< <tt>0x8000</tt> Other elements that don't fit into
...
enum PlottingHint {
    ...
    phFastPolylines =
        0x001  ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster
               ///< method. This reduces the quality especially of the line
               ///< segment
               ///<                joins, thus is most effective for pen sizes
               ///<                larger than 1. It is only used for solid line
               ///<                pens.
    ,
    phImmediateRefresh =
        0x002  ///< <tt>0x002</tt> causes an immediate repaint() instead of a
               ///< soft update() when QCustomPlot::replot() is called with
               ///< parameter \ref QCustomPlot::rpRefreshHint.
               ///<                This is set by default to prevent the plot
               ///<                from freezing on fast consecutive replots
               ///<                (e.g. user drags ranges with mouse).
    ,
...
enum Interaction {
    ...
    iRangeDrag = 0x001  ///< <tt>0x001</tt> Axis ranges are draggable (see \ref
                        ///< QCPAxisRect::setRangeDrag, \ref
                        ///< QCPAxisRect::setRangeDragAxes)
    ,
    iRangeZoom = 0x002  ///< <tt>0x002</tt> Axis ranges are zoomable with the
                        ///< mouse wheel (see \ref QCPAxisRect::setRangeZoom,
                        ///< \ref QCPAxisRect::setRangeZoomAxes)
    ,
    iMultiSelect =
        0x004  ///< <tt>0x004</tt> The user can select multiple objects by
               ///< holding the modifier set by \ref
               ///< QCustomPlot::setMultiSelectModifier while clicking
    ,
    iSelectPlottables =
        0x008  ///< <tt>0x008</tt> Plottables are selectable (e.g. graphs,
               ///< curves, bars,... see QCPAbstractPlottable)
    ,
    ...
    ,
    iSelectOther =
        0x080  ///< <tt>0x080</tt> All other objects are selectable (e.g. your
               ///< own derived layerables, other layout elements,...)
    ,
    iSelectPlottablesBeyondAxisRect =
        0x100  ///< <tt>0x100</tt> When performing plottable selection/hit
               ///< tests, this flag extends the sensitive area beyond the axis
               ///< rect
};
...
enum SelectionRectMode {
    ...
    srmZoom  ///< When dragging the mouse, a selection rect becomes active. Upon
             ...
             ///< zoomed accordingly.
    ,
    srmSelect  ///< When dragging the mouse, a selection rect becomes active.
               ///< Upon releasing, plottable data points that were within the
               ///< selection rect are selected, if the plottable's
               ///< selectability setting permits. (See  \ref dataselection
               ///< "data selection mechanism" for details.)
    ,
    srmCustom  ///< When dragging the mouse, a selection rect becomes active. It
               ///< is the programmer's responsibility to connect according
               ///< slots to the selection rect's signals (e.g. \ref
...
// for newer Qt versions we have to declare the enums/flags as metatypes inside
// the namespace using Q_ENUM_NS/Q_FLAG_NS: if you change anything here, don't
// forget to change it for older Qt versions below, too, and at the start of the
// namespace in the fake moc-run class
...
// for older Qt versions we have to declare the enums/flags as metatypes outside
// the namespace using Q_DECLARE_METATYPE: if you change anything here, don't
// forget to change it for newer Qt versions above, too, and at the start of the
// namespace in the fake moc-run class
...
public:
    /*!
      ...
      certain subsets of features/fixes/workarounds, depending on whether they
      are wanted on the respective output device.
    */
    enum PainterMode {
        pmDefault =
            0x00  ///< <tt>0x00</tt> Default mode for painting on screen devices
        ,
        pmVectorized = 0x01  ///< <tt>0x01</tt> Mode for vectorized painting
                             ///< (e.g. PDF export). For example, this prevents
                             ///< some antialiasing fixes.
        ,
        pmNoCaching = 0x02  ///< <tt>0x02</tt> Mode for all sorts of exports
                            ///< (e.g. PNG, PDF,...). For example, this prevents
                            ///< using cached pixmap labels
        ,
        pmNonCosmetic = 0x04  ///< <tt>0x04</tt> Turns pen widths 0 to 1, i.e.
                              ///< disables cosmetic pens. (A cosmetic pen is
                              ///< always drawn with width 1 pixel in the vector
                              ///< image/pdf viewer, independent of zoom.)
    };
    Q_ENUMS(PainterMode)
    ...
    // methods hiding non-virtual base class functions (QPainter bug
    // workarounds):
...
inline QDebug operator<<(QDebug d, const QCPDataRange &dataRange) {
    d.nospace() << "QCPDataRange(" << dataRange.begin() << ", "
                << dataRange.end() << ")";
    return d;
}
...
class QCP_LIB_DECL QCPLayoutElement : public QCPLayerable {
    ...
    Q_PROPERTY(SizeConstraintRect sizeConstraintRect READ sizeConstraintRect
                   WRITE setSizeConstraintRect)
    /// \endcond
public:
    /*!
      Defines the phases of the update process, that happens just before a
      replot. At each phase, \ref update is called with the according
      UpdatePhase value.
    */
    enum UpdatePhase {
        upPreparation  ///< Phase used for any type of preparation that needs to
                       ///< be done before margin calculation and layout
        ,
        upMargins  ///< Phase in which the margins are calculated and set
        ,
        upLayout  ///< Final phase in which the layout system places the rects
                  ///< of the elements
    };
    Q_ENUMS(UpdatePhase)
...
class QCP_LIB_DECL QCPLayoutGrid : public QCPLayout {
    ...
    Q_PROPERTY(QList<double> columnStretchFactors READ columnStretchFactors
                   WRITE setColumnStretchFactors)
...
public:
    ...
    */
    enum FillOrder {
        foRowsFirst  ///< Rows are filled first, and a new element is wrapped to
                     ///< the next column if the row count would exceed \ref
                     ///< setWrap.
        ,
        foColumnsFirst  ///< Columns are filled first, and a new element is
                        ///< wrapped to the next row if the column count would
                        ///< exceed \ref setWrap.
    };
    Q_ENUMS(FillOrder)
    ...
    */
    enum InsetPlacement {
        ipFree  ///< The element may be positioned/sized arbitrarily, see \ref
                ///< setInsetRect
        ,
        ipBorderAligned  ///< The element is aligned to one of the layout sides,
                         ///< see \ref setInsetAlignment
    };
...
public:
    ...
    */
    enum EndingStyle {
        ...
        ,
        esFlatArrow  ///< A filled arrow head with a straight/flat back (a
                     ///< triangle)
        ,
        ...
        ,
        esHalfBar  ///< A bar perpendicular to the line, pointing out to only
                   ///< one side (to which side can be changed with \ref
                   ///< setInverted)
        ,
...
protected:
    ...
    struct LabelData {
        ...
        QTransform transform;  // the transform about the label anchor which is
                               // at (0, 0). Does not contain final absolute x/y
                               // positioning on the plot/axis
        QString basePart, expPart, suffixPart;
        QRect baseBounds, expBounds, suffixBounds;
        QRect totalBounds;  // is in a coordinate system where label top left is
                            // at (0, 0)
    ...
    int mPadding;
    double
        mRotation;  // this is the rotation applied uniformly to all labels, not
                    // the heterogeneous rotation in amCircularRotated mode
    bool mSubstituteExponent;
    QChar mMultiplicationSymbol;
    bool mAbbreviateDecimalPowers;
    // non-property members:
    QCustomPlot *mParentPlot;
    QByteArray mLabelParameterHash;  // to determine whether mLabelCache needs
                                     // to be cleared due to changed parameters
    ...
    QByteArray cacheKey(const QString &text, const QColor &color,
                        double rotation, AnchorSide side) const;
    ...
    */
    enum TickStepStrategy {
        tssReadability  ///< A nicely readable tick step is prioritized over
                        ...
                        ///< setTickCount)
        ,
        tssMeetTickCount  ///< Less readable tick steps are allowed which in
                          ///< turn facilitates getting closer to the requested
                          ///< tick count
    };
...
public:
    /*!
      Defines the logical units in which fractions of time spans can be
      expressed.

      \see setFieldWidth, setTimeFormat
    */
    enum TimeUnit {
        tuMilliseconds  ///< Milliseconds, one thousandth of a second (%%z in
                        ///< \ref setTimeFormat)
        ,
...
public:
    ...
    */
    enum ScaleStrategy {
        ...
        ,
        ssMultiples  ///< An integer multiple of the specified tick step is
                     ///< allowed. The used factor follows the base class
                     ///< properties of \ref setTickStepStrategy and \ref
                     ///< setTickCount.
        ,
...
public:
    ...
    */
    enum FractionStyle {
        fsFloatingPoint  ///< Fractions are displayed as regular decimal
                         ///< floating point numbers, e.g. "0.25" or "0.125".
        ,
        fsAsciiFractions  ///< Fractions are written as rationals using ASCII
                          ///< characters only, e.g. "1/4" or "1/8"
        ,
        fsUnicodeFractions  ///< Fractions are written using sub- and
                            ///< superscript UTF-8 digits and the fraction
                            ///< symbol.
    };
...
class QCP_LIB_DECL QCPAxis : public QCPLayerable {
    ...
    Q_PROPERTY(QFont selectedLabelFont READ selectedLabelFont WRITE
...
public:
    ...
    */
    enum AxisType {
        atLeft = 0x01  ///< <tt>0x01</tt> Axis is vertical and on the left side
                       ///< of the axis rect
        ,
        atRight = 0x02  ///< <tt>0x02</tt> Axis is vertical and on the right
                        ///< side of the axis rect
        ,
        atTop = 0x04  ///< <tt>0x04</tt> Axis is horizontal and on the top side
                      ///< of the axis rect
        ,
    ...
    */
    enum ScaleType {
        ...
        stLogarithmic  ///< Logarithmic scaling with correspondingly transformed
                       ///< axis coordinates (possibly also \ref setTicker to a
                       ///< \ref QCPAxisTickerLog instance).
    };
    Q_ENUMS(ScaleType)
    /*!
    ...
    */
    enum SelectablePart {
        ...
        ,
        spTickLabels = 0x002  ///< Tick labels (numbers) of this axis (as a
    ...
    static Qt::Orientation orientation(AxisType type) {
        return type == atBottom || type == atTop ? Qt::Horizontal
                                                 : Qt::Vertical;
    }
...
protected:
    ...
    struct TickLabelData {
        QString basePart, expPart, suffixPart;
        QRect baseBounds, expBounds, suffixBounds, totalBounds,
            rotatedTotalBounds;
        QFont baseFont, expFont;
    };
    QCustomPlot *mParentPlot;
    ...
    virtual TickLabelData getTickLabelData(const QFont &font,
                                           const QString &text) const;
    virtual QPointF getTickLabelDrawOffset(
        const TickLabelData &labelData) const;
...
public:
    /*!
      Represents the various properties of a scatter style instance. For
      example, this enum is used to specify which properties of \ref
      QCPSelectionDecorator::setScatterStyle will be used when highlighting
      selected data points.

      Specific scatter properties can be transferred between \ref
      QCPScatterStyle instances via \ref setFromOther.
    */
    ...
    /*!
      ...
      (with exception of \ref ssDot and \ref ssPixmap) can be controlled with
      the \ref setSize function. Scatters are drawn with the pen and brush
      specified with \ref setPen and \ref setBrush.
    */
    enum ScatterShape {
        ssNone  ///< no scatter symbols are drawn (e.g. in QCPGraph, data only
                ///< represented with lines)
        ,
        ssDot  ///< \enumimage{ssDot.png} a single pixel (use \ref ssDisc or
               ///< \ref ssCircle if you want a round shape with a certain
               ///< radius)
        ,
        ...
        ,
        ssDisc  ///< \enumimage{ssDisc.png} a circle which is filled with the
                ///< pen's color (not the brush as with ssCircle)
        ,
        ...
        ,
        ssTriangleInverted  ///< \enumimage{ssTriangleInverted.png} an
                            ///< equilateral triangle, standing on corner
        ,
        ssCrossSquare  ///< \enumimage{ssCrossSquare.png} a square with a cross
                       ///< inside
        ,
        ssPlusSquare  ///< \enumimage{ssPlusSquare.png} a square with a plus
                      ///< inside
        ,
        ssCrossCircle  ///< \enumimage{ssCrossCircle.png} a circle with a cross
                       ///< inside
        ,
        ssPlusCircle  ///< \enumimage{ssPlusCircle.png} a circle with a plus
                      ///< inside
        ,
        ssPeace  ///< \enumimage{ssPeace.png} a circle, with one vertical and
                 ///< two downward diagonal lines
        ,
        ssPixmap  ///< a custom pixmap specified by \ref setPixmap, centered on
    ...
    // setters:
    void setFromOther(const QCPScatterStyle &other,
...
public:
    ...
    QCPRange keyRange(bool &foundRange,
...
void QCPDataContainer<DataType>::add(const QCPDataContainer<DataType> &data) {
    ...
    if (oldSize > 0 &&
        !qcpLessThanSortKey<DataType>(
            *constBegin(),
            *(data.constEnd() - 1)))  // prepend if new data keys are all
                                      // smaller than or equal to existing ones
    {
    ...
    } else  // don't need to prepend, so append and merge if necessary
    {
        ...
        if (oldSize > 0 &&
            !qcpLessThanSortKey<DataType>(
                *(constEnd() - n - 1),
                *(constEnd() -
                  n)))  // if appended range keys aren't all greater than
                        // existing ones, merge the two partitions
...
void QCPDataContainer<DataType>::add(const QVector<DataType> &data,
            ...
            // necessary
    {
        mData.resize(mData.size() + n);
        std::copy(data.constBegin(), data.constEnd(), end() - n);
        if (!alreadySorted)  // sort appended subrange if it wasn't already
                             // sorted
...
void QCPDataContainer<DataType>::add(const DataType &data) {
    ...
    {
        QCPDataContainer<DataType>::iterator insertionPoint = std::lower_bound(
            begin(), end(), data, qcpLessThanSortKey<DataType>);
    ...
    if (it != end() && it->sortKey() == sortKey) {
        if (it == begin())
            ++mPreallocSize;  // don't actually delete, just add it to the
                              // preallocated block (if it gets too large,
                              // squeeze will take care of it)
        else
            mData.erase(it);
    }
    if (mAutoSqueeze) performAutoSqueeze();
...
QCPDataContainer<DataType>::findBegin(double sortKey,
    ...
    QCPDataContainer<DataType>::const_iterator it = std::lower_bound(
        constBegin(), constEnd(), DataType::fromSortKey(sortKey),
        qcpLessThanSortKey<DataType>);
    if (expandedRange &&
...
QCPRange QCPDataContainer<DataType>::keyRange(bool &foundRange,
    ...
    if (signDomain == QCP::sdBoth)  // range may be anywhere
    {
        if (DataType::sortKeyIsMainKey())  // if DataType is sorted by main key
                                           // (e.g. QCPGraph, but not QCPCurve),
                                           // use faster algorithm by finding
                                           // just first and last key with
                                           // non-NaN value
        {
            ...
            it = itEnd;
            while (it !=
                   constBegin())  // find first non-nan going down from right
            {
                --it;
                if (!qIsNaN(it->mainValue())) {
        ...
        } else  // DataType is not sorted by main key, go through all data
                // points and accordingly expand range
        {
            while (it != itEnd) {
                if (!qIsNaN(it->mainValue())) {
                    current = it->mainKey();
    ...
    } else if (signDomain == QCP::sdNegative)  // range may only be in the
                                               // negative sign domain
    {
        while (it != itEnd) {
            if (!qIsNaN(it->mainValue())) {
                current = it->mainKey();
    ...
    } else if (signDomain == QCP::sdPositive)  // range may only be in the
                                               // positive sign domain
    {
        while (it != itEnd) {
            if (!qIsNaN(it->mainValue())) {
                current = it->mainKey();
...
QCPRange QCPDataContainer<DataType>::valueRange(bool &foundRange,
    ...
    if (signDomain == QCP::sdBoth)  // range may be anywhere
    {
        for (QCPDataContainer<DataType>::const_iterator it = itBegin;
             it != itEnd; ++it) {
            ...
            if ((current.lower < range.lower || !haveLower) &&
                !qIsNaN(current.lower) && std::isfinite(current.lower)) {
                range.lower = current.lower;
                haveLower = true;
            }
            if ((current.upper > range.upper || !haveUpper) &&
                !qIsNaN(current.upper) && std::isfinite(current.upper)) {
                                               ...
                                               // negative sign domain
    {
        for (QCPDataContainer<DataType>::const_iterator it = itBegin;
            ...
            current = it->valueRange();
            if ((current.lower < range.lower || !haveLower) &&
                current.lower < 0 && !qIsNaN(current.lower) &&
                std::isfinite(current.lower)) {
                range.lower = current.lower;
                haveLower = true;
            }
            if ((current.upper > range.upper || !haveUpper) &&
                current.upper < 0 && !qIsNaN(current.upper) &&
                std::isfinite(current.upper)) {
                range.upper = current.upper;
                haveUpper = true;
            }
        }
...
void QCPDataContainer<DataType>::preallocateGrow(int minimumPreallocSize) {
    ...
    newPreallocSize += (1u << qBound(4, mPreallocIteration + 4, 15)) -
                       12;  // do 4 up to 32768-12 preallocation, doubling in
...
void QCPDataContainer<DataType>::performAutoSqueeze() {
                              ...
                              // respect to total used size
    {
        shrinkPostAllocation =
            postAllocSize >
            usedSize * 1.5;  // QVector grow strategy is 2^n for static data.
                             // Watch out not to oscillate!
                      ...
                      // memory, below 1k points don't even bother
    {
        ...
        shrinkPreAllocation =
            mPreallocSize >
            usedSize * 1.5;  // preallocation can grow into postallocation, so
                             // can be smaller
    }
...
class QCP_LIB_DECL QCPAbstractPlottable : public QCPLayerable {
    ...
    Q_PROPERTY(QCPDataSelection selection READ selection WRITE setSelection
...
public:
    ...
    virtual double selectTest(const QPointF &pos, bool onlySelectable,
                              QVariant *details = nullptr) const
        Q_DECL_OVERRIDE = 0;  // actually introduced in QCPLayerable as
                              // non-pure, but we want to force reimplementation
                              // for plottables
...
protected:
    ...
    // non-virtual methods:
    void addChildX(QCPItemPosition *pos);  // called from pos when this anchor
                                           // is set as parent
    void removeChildX(QCPItemPosition *pos);  // called from pos when its parent
                                              // anchor is reset or pos deleted
    void addChildY(QCPItemPosition *pos);  // called from pos when this anchor
                                           // is set as parent
...
public:
    /*!
      Defines the ways an item position can be specified. Thus it defines what
      the numbers passed to \ref setCoords actually mean.

      \see setType
    */
    enum PositionType {
        ptAbsolute  ///< Static positioning in pixels, starting from the top
                    ///< left corner of the viewport/widget.
        ,
        ptViewportRatio  ///< Static positioning given by a fraction of the
                         ///< viewport size. For example, if you call
                         ///< setCoords(0, 0), the position will be at the top
                         ///< left corner of the viewport/widget. setCoords(1,
                         ///< 1) will be at the bottom right corner,
                         ///< setCoords(0.5, 0) will be horizontally centered
                         ///< and vertically at the top of the viewport/widget,
                         ///< etc.
        ,
        ptAxisRectRatio  ///< Static positioning given by a fraction of the axis
                         ///< rect size (see \ref setAxisRect). For example, if
                         ///< you call setCoords(0, 0), the position will be at
                         ///< the top left corner of the axis rect. setCoords(1,
                         ///< 1) will be at the bottom right corner,
                         ///< setCoords(0.5, 0) will be horizontally centered
                         ///< and vertically at the top of the axis rect, etc.
                         ///< You can also go beyond the axis rect by providing
                         ///< negative coordinates or coordinates larger than 1.
        ,
        ptPlotCoords  ///< Dynamic positioning at a plot coordinate defined by
                      ///< two axes (see \ref setAxes).
    };
...
class QCP_LIB_DECL QCustomPlot : public QWidget {
    ...
    Q_PROPERTY(Qt::AspectRatioMode backgroundScaledMode READ
                   backgroundScaledMode WRITE setBackgroundScaledMode)
    ...
    */
    enum RefreshPriority {
        rpImmediateRefresh  ///< Replots immediately and repaints the widget
                            ///< immediately by calling QWidget::repaint() after
                            ///< the replot
        ,
        rpQueuedRefresh  ///< Replots immediately, but queues the widget
                         ///< repaint, by calling QWidget::update() after the
                         ///< replot. This way multiple redundant widget
                         ///< repaints can be avoided.
        ,
        rpRefreshHint  ///< Whether to use immediate or queued refresh depends
                       ///< on whether the plotting hint \ref
                       ///< QCP::phImmediateRefresh is set, see \ref
                       ///< setPlottingHints.
        ,
        rpQueuedReplot  ///< Queues the entire replot for the next event loop
                        ///< iteration. This way multiple redundant replots can
                        ///< be avoided. The actual replot is then done with
                        ///< \ref rpRefreshHint priority.
    };
    ...
    void setInteraction(const QCP::Interaction &interaction,
                        bool enabled = true);
    ...
    QCPGraph *addGraph(QCPAxis *keyAxis = nullptr,
...
PlottableType *QCustomPlot::plottableAt(const QPointF &pos, bool onlySelectable,
    ...
    double resultDistance =
        mSelectionTolerance;  // only regard clicks with distances smaller than
                              // mSelectionTolerance as selections, so
                              // initialize with that value

    foreach (QCPAbstractPlottable *plottable, mPlottables) {
        PlottableType *currentPlottable =
            qobject_cast<PlottableType *>(plottable);
        if (!currentPlottable ||
            (onlySelectable &&
             !currentPlottable
                  ->selectable()))  // we could have also passed onlySelectable
    ...
    foreach (QCPAbstractItem *item, mItems) {
        ...
        if (!currentItem->clipToAxisRect() ||
            currentItem->clipRect().contains(
                pos.toPoint()))  // only consider clicks inside axis cliprect of
                                 // the item if actually clipped to it
        {
...
QCPDataSelection QCPAbstractPlottable1D<DataType>::selectTestRect(
    ...
    QCPRange keyRange(key1,
                      key2);  // QCPRange normalizes internally so we don't have
                              // to care about whether key1 < key2
    ...
    if (DataType::sortKeyIsMainKey())  // we can assume that data is sorted by
                                       // main key, so can reduce the searched
                                       // key interval:
    {
    ...
    int currentSegmentBegin = -1;  // -1 means we're currently not in a segment
                                   // that's contained in rect
    for (typename QCPDataContainer<DataType>::const_iterator it = begin;
        ...
        {
            result.addDataRange(
                QCPDataRange(currentSegmentBegin,
...
double QCPAbstractPlottable1D<DataType>::selectTest(const QPointF &pos,
    ...
    if (DataType::sortKeyIsMainKey())  // we can assume that data is sorted by
                                       ...
                                       // key interval:
    {
        // determine which key range comes into question, taking selection
        // tolerance around pos into account:
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin;
        ...
        if (keyRange.contains(mainKey) &&
            valueRange.contains(
                mainValue))  // make sure data point is inside visible range,
                             // for speedup in cases where sort key isn't main
                             // key and we iterate over all points
        {
            const double currentDistSqr =
                QCPVector2D(coordsToPixels(mainKey, mainValue) - pos)
                    .lengthSquared();
    ...
    if (minDistIndex != mDataContainer->size())
        selectionResult.addDataRange(
            QCPDataRange(minDistIndex, minDistIndex + 1), false);
...
void QCPAbstractPlottable1D<DataType>::getDataSegments(
                       ...
                       // selected style if mSelection isn't empty
    {
        ...
        unselectedSegments =
            sel.inverse(QCPDataRange(0, dataCount())).dataRanges();
    }
}
...
void QCPAbstractPlottable1D<DataType>::drawPolyline(
    ...
    // though it has same appearance apart from High-DPI. In High-DPI cases
    // people must set a pen width slightly larger than 1.0 to get correct DPI
    // scaling of width, but of course with performance penalty.
        ...
        while (i < lineDataSize) {
                                   ...
                                   // Infs which make drawPolyline block
            {
                painter->drawPolyline(
                    ...
                    i - segmentStart);  // i, because we don't want to include
                                        // the current NaN point
                segmentStart = i + 1;
            }
            ++i;
        }
...
public:
    /*!
      Defines the color spaces in which color interpolation between gradient
      stops can be performed.
    ...
    */
    enum ColorInterpolation {
        ...
        ciHSV  ///< Color channels hue, saturation and value are linearly
               ///< interpolated (The hue is interpolated over the shortest
               ///< angle distance)
    };
    ...
    */
    enum NanHandling {
        nhNone  ///< NaN data points are not explicitly handled and shouldn't
                ///< occur in the data (this gives slight performance
                ///< improvement)
        ,
        nhLowestColor  ///< NaN data points appear as the lowest color defined
                       ///< in this QCPColorGradient
        ,
        nhHighestColor  ///< NaN data points appear as the highest color defined
                        ///< in this QCPColorGradient
        ,
    ...
    */
    enum GradientPreset {
        ...
        gpCold  ///< Continuous lightness from black over icey colors to white
                ///< (suited for non-biased data representation)
        ,
        gpNight  ///< Continuous lightness from black over weak blueish colors
                 ///< to white (suited for non-biased data representation)
        ,
        ...
        gpIon  ///< Half hue spectrum from black over purple to blue and finally
               ...
               ///< magnitude estimates)
        ,
        gpThermal  ///< Colors suitable for thermal imaging, ranging from dark
                   ///< blue over purple to orange, yellow and white
        ,
        gpPolar  ///< Colors suitable to emphasize polarity around the center,
                 ///< with blue for negative, black in the middle and red for
                 ///< positive values
        ,
        ...
        gpJet  ///< Hue variation similar to a spectrum, often used in numerical
               ///< visualization (creates banding illusion but allows more
               ///< precise magnitude estimates)
        ,
        gpHues  ///< Full hue cycle, with highest and lowest color red (suitable
                ///< for periodic data, such as angles and phases, see \ref
                ///< setPeriodic)
    };
...
public:
    ...
    */
    enum BracketStyle {
        ...
        ,
        bsEllipse  ///< An ellipse is drawn. The size of the ellipse is given by
                   ///< the bracket width/height properties.
        ,
        bsPlus  ///< A plus is drawn.
        ,
        bsUserStyle  ///< Start custom bracket styles at this index when
                     ///< subclassing and reimplementing \ref drawBracket.
    };
...
public:
    ...
    void setRangeDragAxes(QList<QCPAxis *> horizontal,
                          QList<QCPAxis *> vertical);
    void setRangeZoomAxes(QCPAxis *horizontal, QCPAxis *vertical);
    void setRangeZoomAxes(QList<QCPAxis *> axes);
    void setRangeZoomAxes(QList<QCPAxis *> horizontal,
                          QList<QCPAxis *> vertical);
...
class QCP_LIB_DECL QCPLegend : public QCPLayoutGrid {
    ...
    Q_PROPERTY(QPen selectedBorderPen READ selectedBorderPen WRITE
...
protected:
    ...
    // re-using some methods of QCPAxisRect to make them available to friend
    // class QCPColorScale
...
public:
    /*!
      ...
      is drawn with the current pen of the graph (\ref setPen). \see
      setLineStyle
    */
    enum LineStyle {
        lsNone  ///< data points are not connected with any lines (e.g. data
                ///< only represented with symbols according to the scatter
                ///< style, see \ref setScatterStyle)
        ,
        lsLine  ///< data points are connected by a straight line
        ,
        lsStepLeft  ///< line is drawn as steps where the step height is the
                    ///< value of the left data point
        ,
        lsStepRight  ///< line is drawn as steps where the step height is the
                     ///< value of the right data point
        ,
        lsStepCenter  ///< line is drawn as steps where the step is in between
                      ///< two data points
        ,
...
protected:
    ...
    QVector<QPointF> dataToImpulseLines(
        const QVector<QCPGraphData> &data) const;
    QVector<QCPDataRange> getNonNanSegments(
...
public:
    /*!
      ...
      is drawn with the current pen of the curve (\ref setPen). \see
      setLineStyle
    */
    enum LineStyle {
...
protected:
    ...
    QPointF getOptimizedPoint(int otherRegion, double otherKey,
...
public:
    ...
    */
    enum SpacingType {
        ...
        ,
        stAxisRectRatio  ///< Bar spacing is given by a fraction of the axis
                         ///< rect size
        ,
        stPlotCoords  ///< Bar spacing is in key coordinates and thus scales
                      ///< with the key axis range
    };
    Q_ENUMS(SpacingType)
...
public:
    /*!
      Defines the ways the width of the bar can be specified. Thus it defines
      what the number passed to \ref setWidth actually means.
    ...
    */
    enum WidthType {
        ...
        ,
        wtAxisRectRatio  ///< Bar width is given by a fraction of the axis rect
                         ///< size
        ,
        wtPlotCoords  ///< Bar width is in key coordinates and thus scales with
                      ///< the key axis range
    };
    Q_ENUMS(WidthType)
...
public:
    ...
    void addData(const QVector<double> &keys, const QVector<double> &minimum,
                 ...
                 const QVector<double> &upperQuartile,
                 const QVector<double> &maximum, bool alreadySorted = false);
    void addData(double key, double minimum, double lowerQuartile,
                 double median, double upperQuartile, double maximum,
                 const QVector<double> &outliers = QVector<double>());
...
public:
    ...
    }  // open and close must lie between low and high, so we don't need to
       // check them
...
public:
    ...
    */
    enum WidthType {
        ...
        ,
        wtPlotCoords  ///< width is in key coordinates and thus scales with the
                      ///< key axis range
    };
    Q_ENUMS(WidthType)
    ...
    */
    ...
    void addData(const QVector<double> &keys, const QVector<double> &open,
                 const QVector<double> &high, const QVector<double> &low,
                 const QVector<double> &close, bool alreadySorted = false);
    void addData(double key, double open, double high, double low,
                 double close);
...
public:
    /*!
      Defines in which orientation the error bars shall appear. If your data
      needs both error dimensions, create two \ref QCPErrorBars with different
      \ref ErrorType.

      \see setErrorType
    */
    enum ErrorType {
        etKeyError  ///< The errors are for the key dimension (bars appear
...
class QCP_LIB_DECL QCPItemPixmap : public QCPAbstractItem {
    ...
    Q_PROPERTY(
        Qt::TransformationMode transformationMode READ transformationMode)
    Q_PROPERTY(QPen pen READ pen WRITE setPen)
    Q_PROPERTY(QPen selectedPen READ selectedPen WRITE setSelectedPen)
    /// \endcond
public:
...
public:
    ...
    */
    enum TracerStyle {
        ...
        ,
        tsCrosshair  ///< A plus shaped crosshair which spans the complete axis
                     ///< rect
        ,
        tsCircle  ///< A circle
        ,
...
public:
    /*!
      Defines the various visual shapes of the bracket item. The appearance can
      be further modified by \ref setLength and \ref setPen.
...
public:
    ...
    */
    enum AngleReference {
        arAbsolute  ///< The axis tilt is given in absolute degrees. The zero is
                    ///< to the right and positive angles are measured
                    ///< counter-clockwise.
        ,
...
public:
    ...
    */
    ...
    virtual double selectTest(const QPointF &pos, bool onlySelectable,
                              QVariant *details = 0)
        const;  // actually introduced in QCPLayerable as non-pure, but we want
