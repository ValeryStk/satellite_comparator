...
QPair<bool, QJsonObject> isJsonObjectContainsUserKeys(
    const QJsonObject& user_json_from_file, const QJsonArray& service_keys,
        ...
        for (int j = 0; j < jarr_custom_keys.size(); ++j) {
            ...
            if (user_json_from_file.contains(custom_key)) {
                if (user_json_from_file[custom_key].isArray()) {
                    result_object_with_data.insert(
                        service_keys[i].toString(),
...
QVector<QStringList> getStringListFromJsonArray(const QJsonArray& array) {
    ...
    for (int i = 0; i < array.size(); ++i) {
        if (array[i].isArray() == false) continue;
        QStringList list;
...
QString toString(const QJsonValue& value) {
    ...
    if (value.isObject())
        ...
        jsonDoc = QJsonDocument(value.toArray());
