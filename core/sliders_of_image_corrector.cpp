...
inline bool get_slider_value_from_position(const int action, QSlider *slider) {
    ...
    if (action == AS::SliderPageStepAdd || action == AS::SliderPageStepSub) {
        int value = slider->style()->sliderValueFromPosition(
            ...
            slider->mapFromGlobal(QCursor::pos()).x(), slider->width());
...
inline double calculate_proportion_coefficient(
    const int base_range_value, const std::pair<double, double> base_range,
    ...
    if (base_range_value < init_val) {
        double t =
            static_cast<double>(init_val - base_range_value) / (init_val - brs);
        return neutral_multiplier - t * (neutral_multiplier - mrs);
    }
    if (base_range_value > init_val) {
        double t =
            static_cast<double>(base_range_value - init_val) / (bre - init_val);
...
inline double calculate_slider_coef(const QSlider *slider) {
    return calculate_proportion_coefficient(
        slider->value(), {SLIDER_MIN_VALUE, SLIDER_MAX_VALUE},
...
SlidersOfImageCorrector::SlidersOfImageCorrector(QWidget *parent)
    : QWidget(parent), ui(new Ui::SlidersOfImageCorrector) {
    ui->setupUi(this);
    ui->label_light->setVisible(false);
    ui->label_saturation->setVisible(false);
    ui->label_saturation_text->setPixmap(
        QPixmap::fromImage(QImage(":/res/saturation.svg")));
    ui->label_light_text->setPixmap(
        QPixmap::fromImage(QImage(":/res/light.svg")));
    ...
    connect(ui->slider_saturation, SIGNAL(sliderReleased()), this,
...
void SlidersOfImageCorrector::showToolTip(QSlider *slider, const int value) {
