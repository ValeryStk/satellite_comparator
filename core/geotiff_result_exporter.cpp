...
bool GeoTiffResultExporter::writeSubstrateGeoTiff(
    const QString &filePath, const QImage &baseImageIn,
    const sad::geoTransform &geo) {
    ...
    const int xSize = baseImage.width();
    const int ySize = baseImage.height();
    ...
    for (int bandIdx = 0; bandIdx < 3; ++bandIdx) {
        for (int y = 0; y < ySize; ++y) {
            const uchar *line = baseImage.constScanLine(y);
            for (int x = 0; x < xSize; ++x) {
                channelBuf[y * xSize + x] = line[x * 3 + bandIdx];
            }
        }
...
bool GeoTiffResultExporter::exportSearchResult(
    ...
    const ClassRaster raster = buildClassRaster(maskImage);
    if (raster.xSize <= 0) {
    ...
    if (!writeClassGeoTiff(filePath, raster, geoTransform)) {
    ...
    if (options.exportSubstrate && !baseImage.isNull()) {
        ...
        writeSubstrateGeoTiff(substratePath, baseImage, geoTransform);
    }
