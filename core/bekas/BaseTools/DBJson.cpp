...
bool checkNodes(const QJsonObject &obj, const QStringList &nodes) {
    for (int i = 0; i < nodes.size(); ++i) {
        if (obj.find(nodes[i]) == obj.end()) return false;
    }
    return true;
};
...
const QStringList IMAGE_TYPES_DESCRIPTION = {"Не задано", "Обзорное",
                                             "Визирное"};

const QStringList WIND_DIRECTION_DESCRIPTION = {
    "Не задано", "Северо-западный", "Северный", "Северо-восточный",
    "Восточный", "Юго-восточный",   "Южный",    "Юго-западный",
    "Западный"};

const QStringList CAPTURE_LEVEL_DESCRIPTION = {
    "Не задан", "Лабораторный", "Наземный", "Авиационный", "Космический"};

const QStringList SPECTRUM_UNITS_DESCRIPTION = {
    "Не задано", "ед. АЦП", "КСЯ, отн. ед.", "СПЭЯ, Вт/(м³·ср)"};

const QStringList BANDS_UNITS_DESCRIPTION = {"Не задано", "Номер канала",
                                             "Длина волны, нм"};
...
SPECTRAL_STRUCT getStructFromJsonObject(const QJsonObject &json_object) {
    ...
    spectral_struct.md.date_time = QDateTime::fromString(
        md.find("date_time").value().toString(), "yyyy_MM_dd_hh_mm_ss_zzz");
    spectral_struct.md.owner = md.find("owner").value().toString();
    spectral_struct.md.sun_elevation_angle =
        md.find("sun_elevation_angle").value().toDouble();
    spectral_struct.md.capture_angle =
        md.find("capture_angle").value().toDouble();
    spectral_struct.md.experiment_name =
        md.find("experiment_name").value().toString();
    spectral_struct.md.capture_level =
        getCaptureLevelFromStr(md.find("capture_type").value().toString());
    auto classification = md.find("classification")->toObject();
    spectral_struct.md.classification.general_type =
        classification.find("general_type")->toString();
    spectral_struct.md.classification.class_name =
        classification.find("class_name")->toString();
    spectral_struct.md.classification.object_name =
        classification.find("object_name")->toString();
    auto location = md.find("location")->toObject();
    spectral_struct.md.location.latitude =
        location.find("latitude").value().toDouble();
    spectral_struct.md.location.longitude =
        location.find("longitude").value().toDouble();
    spectral_struct.md.location.altitude =
        location.find("altitude").value().toDouble();
    spectral_struct.md.location.local_name =
        location.find("local_name").value().toString();
    spectral_struct.md.location.place_name =
        location.find("place_name").value().toString();
    spectral_struct.md.location.place_type =
        location.find("place_type").value().toString();
    spectral_struct.md.location.region_name =
        location.find("region_name").value().toString();
    ...
    for (auto &&it : images) {
        ...
        img_obj.type =
            getImageTypeFromStr(image_object.find("type")->toString());
    ...
    spectral_struct.md.air_conditions.temperature =
        air_conditions.find("temperature").value().toDouble();
    spectral_struct.md.air_conditions.humidity =
        air_conditions.find("humidity").value().toDouble();
    auto weather_conditions = md.find("weather_conditions").value().toObject();
    spectral_struct.md.weather_conditions.clouds_level =
        weather_conditions.find("clouds_level")->toDouble();
    spectral_struct.md.weather_conditions.wind =
        weather_conditions.find("wind")->toDouble();
    spectral_struct.md.weather_conditions.wind_direction =
        getWindDirectionFromStr(
            weather_conditions.find("direction")->toString());
...
bool isAllNodesExist(const QJsonObject &object) {
    QStringList nodes = {"meta_data", "spectral_data"};
...
void getJsonObjectFromStruct(const SPECTRAL_STRUCT &spectral_struct,
    ...
    meta_data_object["date_time"] =
        spectral_struct.md.date_time.toString("yyyy_MM_dd_hh_mm_ss_zzz");
    meta_data_object["owner"] = spectral_struct.md.owner;
    meta_data_object["sun_elevation_angle"] =
        spectral_struct.md.sun_elevation_angle;
    ...
    for (int i = 0; i < spectral_struct.md.images.size(); ++i) {
        ...
        images_array.append(QJsonObject(
            {{"name", spectral_struct.md.images[i].pathToFile},
             {"type",
              IMAGE_TYPES_DESCRIPTION.at(spectral_struct.md.images[i].type)},
    ...
    weather_conditions["clouds_level"] =
        spectral_struct.md.weather_conditions.clouds_level;
    weather_conditions["wind"] = spectral_struct.md.weather_conditions.wind;
    weather_conditions["direction"] = WIND_DIRECTION_DESCRIPTION.at(
        spectral_struct.md.weather_conditions.wind_direction);
    ...
    for (int i = 0; i < spectral_struct.sd.attributes.size(); ++i) {
        attributes.append(QJsonValue(
            {{"instrument", spectral_struct.sd.attributes[i].instrument},
             {"type", SPECTRUM_UNITS_DESCRIPTION.at(
                          spectral_struct.sd.attributes[i].type)},
...
QString getStringMdFromStruct(META_DATA metaData) {
    ...
    mdString.append("Уровень съемки:\t" +
                    CAPTURE_LEVEL_DESCRIPTION.at(metaData.capture_level) +
                    "\n");
    mdString.append("Дата:\t" +
                    metaData.date_time.date().toString("dd.MM.yyyy") + "\n");
    mdString.append("Время:\t" +
                    metaData.date_time.time().toString("hh:mm:ss.zzz") + "\n");
    ...
    if (metaData.location.latitude > -90 && metaData.location.latitude < 90)
        mdString.append("Широта:\t" +
                        QString::number(metaData.location.latitude, 'f', 2) +
                        "\n");
    if (metaData.location.longitude > -180 && metaData.location.longitude < 180)
        mdString.append("Долгота:\t" +
                        QString::number(metaData.location.longitude, 'f', 2) +
                        "\n");
    if (metaData.location.altitude > 0)
        mdString.append("Высота, м н.у.м.:\t" +
                        QString::number(metaData.location.altitude, 'f', 2) +
                        "\n");
    mdString.append("Угол спектрометрирования, град.:\t" +
                    QString::number(metaData.capture_angle, 'f', 2) + "\n");
    mdString.append("Угол Солнца, град.:\t" +
                    QString::number(metaData.sun_elevation_angle, 'f', 2) +
                    "\n");
    mdString.append(
        "Температура, °С:\t" +
        QString::number(metaData.air_conditions.temperature, 'f', 2) + "\n");
    mdString.append("Влажность, %:\t" +
                    QString::number(metaData.air_conditions.humidity, 'f', 2) +
                    "\n");
    mdString.append("Уровень облачности:\t" +
                    QString::number(metaData.weather_conditions.clouds_level) +
                    "\n");
    mdString.append("Ветер, м/с:\t" +
                    QString::number(metaData.weather_conditions.wind));
    if (metaData.weather_conditions.wind_direction != WD_UNKNOWN)
        mdString.append(", " +
                        WIND_DIRECTION_DESCRIPTION.at(
                            metaData.weather_conditions.wind_direction) +
                        "\n\n");
    else
        mdString.append("\n\n");
...
QString getStringSpecAttrsFromStruct(SPECTRAL_ATTRIBUTES specAttrs) {
    ...
    saString.append("Единицы измерения по оси Y:\t" +
                    SPECTRUM_UNITS_DESCRIPTION.at(specAttrs.type) + "\n");
