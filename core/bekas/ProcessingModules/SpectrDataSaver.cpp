...
void SpectrDataSaver::createFileAndWrite(QString absoluteFileName,
                                         QString *strData) {
...
QString SpectrDataSaver::savingDirectory() const { return m_savingDirectory; }
...
QString SpectrDataSaver::getStringSpectrumFromVectors(QVector<double> waves,
                                                      QVector<double> values) {
    QString specStr;
    if (waves.count() == values.count()) {
        for (int i = 0; i < waves.count(); i++) {
            specStr.append(QString::number(waves.at(i), 'f', 2) + "\t" +
                           QString::number(values.at(i), 'f', 4) + "\n");
        }
    }
    return specStr;
}
...
void SpectrDataSaver::saveStrInSeparateThread(QString baseFileName,
                                              QString strData) {
    emit sendTextMessage("Сохранение файла спектра: " + m_savingDirectory +
                         "/" + baseFileName + ".txt");
...
void SpectrDataSaver::runStrFileSaving(QString baseFileName, QString savingDir,
                                       QString strData) {
...
void SpectrDataSaver::runStrFileSaving(QString absoluteFileName,
                                       QString strData) {
    createFileAndWrite(absoluteFileName, &strData);
}
