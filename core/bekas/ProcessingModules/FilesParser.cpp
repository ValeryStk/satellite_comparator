...
void FilesParser::saveCurrentSpectrum() {
    db_json::saveStructToJsonFile(
        m_spectraPathesList.at(m_currentSpectrumIndex), m_spectrum,
        QJsonDocument::JsonFormat::Indented);
}

db_json::META_DATA FilesParser::getCurrentMetadata() { return m_spectrum.md; }
...
QVector<double> FilesParser::getRflWaves() { return m_spectrum.sd.waves.at(0); }
...
const QString FilesParser::currentSpName() const {
    QString spName =
        m_spectrum.md.date_time.toString("dd.MM.yyyy hh:mm:ss.zzz");
    return spName;
}
...
const QString &FilesParser::inputDirPath() const { return m_inputDirPath; }
...
void FilesParser::removeDataAtIndex(int dataIndexInList) {
    ...
    QJsonObject object;
    if (db_json::getJsonObjectFromFile(spectrumPath, object)) {
        db_json::SPECTRAL_STRUCT spectrum =
            db_json::getStructFromJsonObject(object);
        imagePath =
            m_inputDirPath + "/" + spectrum.md.images.first().pathToFile;
    } else {
        emit sendMessage("Невозможно открыть файл спектра:\n" + spectrumPath);
    }
