...
CustomStringListModel::CustomStringListModel(const QStringList &strings,
                                             QObject *parent)
    : QStringListModel(strings, parent) {}

void CustomStringListModel::setColorForName(const QString &name,
                                            const QColor &color) {
...
QColor CustomStringListModel::getColorForName(const QString &name) const {
    return m_colorMap.value(
        name, QColor());  // Возвращает невалидный QColor если не найдено
}

void CustomStringListModel::setClusterForName(const QString &name,
                                              int cluster) {
...
UasvViewWindow::UasvViewWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::UasvViewWindow) {
    ui->setupUi(this);
    ...
    double startShowingWave =
        m_settings->value("Settings/ShowingStartWave").toDouble();
    double endShowingWave =
        m_settings->value("Settings/ShowingEndWave").toDouble();
    if (!m_plotterWidget->bands().isEmpty()) {
        m_plotterWidget->setBandsRange(startShowingWave, endShowingWave);
    }
}

void UasvViewWindow::setUDPobj(UdpJsonRpc *rpc) { m_rpc = rpc; }

void UasvViewWindow::updateListWithClustNums(
    const QStringList &specNames, const QVector<int> &clusters,
...
void UasvViewWindow::setupProject() {
    m_wTitle.append(VER_PRODUCTNAME_STR)
        .append(" v_")
...
void UasvViewWindow::setupGui() {
    ...
    ui->listViewSpectraNames->setEditTriggers(
        QAbstractItemView::NoEditTriggers);
}

void UasvViewWindow::initObjects() {
    ui->lineEditInputPath->setText(
        m_settings->value("Pathes/InputPath").toString());
...
void UasvViewWindow::updateDataInGui() {
    if (m_slModel->stringList().count() > 0) {
        ...
        QVector<double> values =
            m_filesParser->getRflSpectrumValues(maxInSpectrum);
        drawSpectrum(waves, values, maxInSpectrum, db_json::BU_WAVELENGTH,
                     db_json::SU_RFL, m_filesParser->currentSpName());
...
void UasvViewWindow::fillListOfSpectra() {
    ...
    if (!m_slModel->stringList().isEmpty())
        ui->listViewSpectraNames->setCurrentIndex(
            ui->listViewSpectraNames->indexAt(QPoint(0, 0)));
...
void UasvViewWindow::fillMetaData(db_json::META_DATA spectrumMd) {
    ui->labelSpectrumName->setText(
        spectrumMd.owner + ": " +
        spectrumMd.date_time.toString("yyyy.MM.dd hh:mm:ss.zzz"));
    ...
    ui->labelCaptureLevel->setText(
        db_json::CAPTURE_LEVEL_DESCRIPTION.at(spectrumMd.capture_level));
    ui->labelPlace->setText(spectrumMd.location.place_name);

    ui->labelSunElevation->setText(
        QString::number(spectrumMd.sun_elevation_angle, 'g', 2));
    ui->labelClouds->setText(
        QString::number(spectrumMd.weather_conditions.clouds_level));
    ui->labelHumidity->setText(
        QString::number(spectrumMd.air_conditions.humidity));
    ui->labelTemperature->setText(
        QString::number(spectrumMd.air_conditions.temperature));
    ui->labelWindIntencity->setText(
        QString::number(spectrumMd.weather_conditions.wind));
}
...
void UasvViewWindow::checkClassifAndSetupCBox(QString spClassification,
                                              QComboBox *settingCBox,
                                              QString textForEmpty) {
    if (spClassification.isEmpty()) {
        int defaultIndex = settingCBox->findText(textForEmpty);
        if (defaultIndex != -1) settingCBox->setCurrentIndex(defaultIndex);
    } else {
...
void UasvViewWindow::on_pushButtonChangeInputFolder_clicked() {
    ...
    if (existingDir.isReadable()) {
        checkPath = QFileDialog::getExistingDirectory(
            this, tr("Выбор папки для загрузки"), ui->lineEditInputPath->text(),
            QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    } else {
        checkPath = QFileDialog::getExistingDirectory(
            this, tr("Выбор папки для загрузки"), "/home",
            QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    }
...
void UasvViewWindow::on_pushButtonShowPattBright_clicked() {
    if (ui->pushButtonShowPattBright->isChecked()) {
        ...
        QVector<double> values =
            m_filesParser->getBrightPattSpectrumValues(maxInSpectrum);
        drawSpectrum(waves, values, maxInSpectrum, db_json::BU_WAVELENGTH,
                     db_json::SU_BRIGHT, m_filesParser->currentSpName());
...
void UasvViewWindow::on_pushButtonShowTemplBright_clicked() {
    if (ui->pushButtonShowTemplBright->isChecked()) {
        ...
        QVector<double> values =
            m_filesParser->getBrightTemplSpectrumValues(maxInSpectrum);
        drawSpectrum(waves, values, maxInSpectrum, db_json::BU_WAVELENGTH,
                     db_json::SU_BRIGHT, m_filesParser->currentSpName());
        ui->pushButtonShowRfl->setChecked(false);
        ui->pushButtonShowPattBright->setChecked(false);
...
void UasvViewWindow::on_pushButtonSaveGraph_clicked() {
    ...
    QString fileName = m_filesParser->spectraNamesList().at(
        ui->listViewSpectraNames->currentIndex().row());
    int currSpecIndex;
    QString currSpectrumName = getSpecFileNameSuffix(currSpecIndex);
    m_plotterWidget->customPlot()->saveJpg(basePath + fileName +
                                           currSpectrumName + ".jpg");
}

void UasvViewWindow::on_pushButtonExportAsText_clicked() {
    /*QString basePath = ui->lineEditInputPath->text() + "/";
    QString fileName =
    m_filesParser->spectraNamesList().at(ui->listViewSpectraNames->currentIndex().row());
    ui->graphicsViewImage->saveImageWithFov(basePath + fileName + ".bmp");

    int currSpecIndex;
    QString currSpectrumName = getSpecFileNameSuffix(currSpecIndex);
    QString spectrumMd =
    db_json::getStringMdFromStruct(m_filesParser->spectrum().md); QString
    spectrumAttrs =
    db_json::getStringSpecAttrsFromStruct(m_filesParser->spectrum().sd.attributes.at(currSpecIndex));
    QString spectrumWV =
    SpectrDataSaver::getStringSpectrumFromVectors(m_plotterWidget->showingBands(),
                                                                      m_plotterWidget->showingValues());*/
    // qDebug()<<"spectrum_data: "<<spectrumWV;
    /*SpectrDataSaver textSaver;
    connect(&textSaver, SIGNAL(sendTextMessage(QString)), ui->statusbar,
    SLOT(showMessage(QString))); textSaver.setSingleFileName(basePath + fileName
    + currSpectrumName + ".txt"); textSaver.saveStrInSeparateThread(spectrumMd +
    spectrumAttrs + spectrumWV);*/
}
...
void UasvViewWindow::on_pushButtonSetWaveRange_clicked() {
    ...
    double endShowingWave =
        m_settings->value("Settings/ShowingEndWave").toDouble();
    WavesRangeDialog wavesDialog(
        this, m_plotterWidget->bands(), m_plotterWidget->values(),
        m_plotterWidget->currentMaxValue(), m_plotterWidget->bandUnits(),
        m_plotterWidget->getSpectrumUnits(), startShowingWave, endShowingWave);
    int ret = wavesDialog.exec();
    if (ret == 1) {
        m_plotterWidget->setBandsRange(wavesDialog.minWave(),
                                       wavesDialog.maxWave());
        m_settings->setValue("Settings/ShowingStartWave",
                             wavesDialog.minWave());
...
void UasvViewWindow::on_pushButtonPrevious_clicked() {
    if (ui->listViewSpectraNames->currentIndex().row() - 1 >= 0) {
        ui->listViewSpectraNames->setCurrentIndex(m_slModel->index(
            ui->listViewSpectraNames->currentIndex().row() - 1, 0));
        m_filesParser->setCurrentSpectrumIndex(
            ui->listViewSpectraNames->currentIndex().row());
        updateDataInGui();
    } else {
...
void UasvViewWindow::on_pushButtonNext_clicked() {
    if (ui->listViewSpectraNames->currentIndex().row() + 1 <
        m_slModel->rowCount()) {
        ui->listViewSpectraNames->setCurrentIndex(m_slModel->index(
            ui->listViewSpectraNames->currentIndex().row() + 1, 0));
        m_filesParser->setCurrentSpectrumIndex(
...
void UasvViewWindow::on_listViewSpectraNames_activated(
