...
public:
     ...
     */
    void changePlotStyle(QColor backgroundColor, QColor fontColor,
                         QColor graphColor);
    ...
    /**
     * @brief setIsNeedToShowPoints Function to set is need to show points on
     * graph
    ...
    /**
     * @brief setIsNeedToShowSpectrum   Function needed to set is it needed to
     * update showing spectrum
...
public slots:
     ...
     */
    void showSpectrum(QVector<double> data, double max, bool isNeedToUpdate,
                      QString spName = "");
    void showSpectrum(QVector<unsigned int> data, unsigned int max,
                      bool isNeedToUpdate, QString spName = "");
    void showSpectrum(QVector<unsigned short> data, unsigned short max,
                      bool isNeedToUpdate, QString spName = "");
    void showSpectrum(QVector<unsigned long> data, unsigned long max,
                      bool isNeedToUpdate, QString spName = "");

    /**
     * @brief on_actionShowValues_triggered Slot for 'actionShowValues'
     * 'triggered' signal
     * @param checked
     */
...
private:
    ...
    /**
     * @brief updateSpectrumView    Function to show spectrum in the plotter
     * widget
     ...
     */
    template <typename T>
    void updateSpectrumView(QVector<T> data, T max, bool isNeedToUpdate,
                            QString spName = "") {
        ...
        if (m_isNeedToShowSpectrum) {
            if (m_bands.count() == data.count()) {
                ...
                // qDebug()<<"bands
                // count:"<<m_showingBands.count()<<m_bands.count();
                m_customPlot->graph(0)->setData(m_bands,
                                                fillSpectrumVector(data));
                ...
                if (m_isNeedToShowPoints)
                    m_customPlot->graph(0)->setScatterStyle(QCPScatterStyle(
                        QCPScatterStyle::ssCrossCircle, QPen(Qt::black, 0),
    ...
    /**
     * @brief fillSpectrumVector    Function to form the double vector from
     * template vector
    ...
    QVector<double> fillSpectrumVector(QVector<T> dataVector) {
        ...
        foreach (T value, dataVector) {
            ...
            if (index >= m_minBandIndex &&
                m_showingValues.count() < m_showingBands.count()) {
                m_showingValues.append(value);
                if (m_currentMaxValue < value) m_currentMaxValue = value;
            }
            index++;
        }
        return m_values;
    ...
    QCPItemStraightLine
        *m_horizLine;     //!< Horizontal line in values showing mode
    ...
    db_json::BandUnits
        m_bandUnits;  //!< Vands values (BAND_NUMBERS | WAVELENGTH)
    db_json::SpectrumUnits
        m_spectrumUnits;  //!< Graph values (SU_ADC | SU_RFL | SU_BRIGHT)
