...
void SpectrPlotterWidget::setupPlot() {
    ...
    m_textValues->setVisible(false);  // т.к. есть баг, что при создании item'ов
                                      // они сразу отрисовываются в (0,0)
...
void SpectrPlotterWidget::updateSpectrumView() {
    m_customPlot->xAxis->setRange(m_showingBands.first(),
                                  m_showingBands.last());
...
int SpectrPlotterWidget::minBandIndex() const { return m_minBandIndex; }
...
void SpectrPlotterWidget::setBandsRange(double minBand, double maxBand) {
    ...
    m_maxBand = maxBand;
    fillShowingBands();
    updateSpectrumView(m_values, m_currentMaxValue, false,
                       m_customPlot->graph(0)->name());
}
...
const QVector<double> &SpectrPlotterWidget::values() const { return m_values; }

const QVector<double> &SpectrPlotterWidget::bands() const { return m_bands; }

void SpectrPlotterWidget::setIsNeedToShowSpectrum(
    bool newIsNeedToShowSpectrum) {
    m_isNeedToShowSpectrum = newIsNeedToShowSpectrum;
}

void SpectrPlotterWidget::setGraphValues(
    const db_json::SpectrumUnits &graphValues) {
...
void SpectrPlotterWidget::setIsNeedToShowPoints(bool isNeedToShowPoints) {
    ...
    for (int i = 0; i < m_customPlot->graphCount(); i++) {
        m_customPlot->graph(i)->setScatterStyle(
            QCPScatterStyle(QCPScatterStyle::ssCrossCircle, QPen(Qt::black, 0),
                            QColor(90, 83, 138, 50), 10));
    }
    m_customPlot->replot();
}

void SpectrPlotterWidget::setZoomType(
    QFlags<Qt::Orientations::enum_type> orientation) {
    m_customPlot->axisRect()->setRangeZoom(orientation);
}

void SpectrPlotterWidget::setBands(const QVector<double> bands,
                                   db_json::BandUnits bandsValues) {
    m_bands = bands;
    m_bandUnits = bandsValues;
    fillShowingBands();
}

void SpectrPlotterWidget::changePlotStyle(QColor backgroundColor,
                                          QColor fontColor, QColor graphColor) {
...
void SpectrPlotterWidget::lockSpectrumFeatures() {
    disconnect(m_customPlot, SIGNAL(customContextMenuRequested(QPoint)), this,
               SLOT(contextMenuRequest(QPoint)));
    disconnect(m_customPlot, SIGNAL(mouseMove(QMouseEvent *)), this,
               SLOT(mouseMoveRequest(QMouseEvent *)));
}

void SpectrPlotterWidget::unlockSpectrumFeatures() {
    connect(m_customPlot, SIGNAL(customContextMenuRequested(QPoint)), this,
            SLOT(contextMenuRequest(QPoint)));
    connect(m_customPlot, SIGNAL(mouseMove(QMouseEvent *)), this,
            SLOT(mouseMoveRequest(QMouseEvent *)));
}

QCustomPlot *SpectrPlotterWidget::customPlot() const { return m_customPlot; }

void SpectrPlotterWidget::showSpectrum(QVector<double> data, double max,
                                       bool isNeedToUpdate, QString spName) {
    updateSpectrumView(data, max, isNeedToUpdate, spName);
}

void SpectrPlotterWidget::showSpectrum(QVector<unsigned int> data,
                                       unsigned int max, bool isNeedToUpdate,
                                       QString spName) {
    updateSpectrumView(data, max, isNeedToUpdate, spName);
}

void SpectrPlotterWidget::showSpectrum(QVector<unsigned short> data,
                                       unsigned short max, bool isNeedToUpdate,
                                       QString spName) {
    updateSpectrumView(data, max, isNeedToUpdate, spName);
}

void SpectrPlotterWidget::showSpectrum(QVector<unsigned long> data,
                                       unsigned long max, bool isNeedToUpdate,
                                       QString spName) {
    updateSpectrumView(data, max, isNeedToUpdate, spName);
}
...
void SpectrPlotterWidget::contextMenuRequest(QPoint pos) { Q_UNUSED(pos); }
...
void SpectrPlotterWidget::mouseMoveRequest(QMouseEvent *e) {
    ...
    if (m_isNeedToShowValues) {
        m_textValues->position->setCoords(
            x, m_customPlot->yAxis->pixelToCoord(e->y() - 10));
