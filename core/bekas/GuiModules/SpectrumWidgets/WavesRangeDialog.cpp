...
WavesRangeDialog::WavesRangeDialog(QWidget *parent, QVector<double> waves,
                                   QVector<double> values, double maxValue,
                                   db_json::BandUnits bandsUnits,
                                   db_json::SpectrumUnits spUnits,
                                   double minWave, double maxWave)
    : QDialog(parent), ui(new Ui::WavesRangeDialog) {
    ui->setupUi(this);
    ...
    if ((maxWave - minWave) > 10) {
        ...
        ui->horizontalSliderMinBand->setValue(
            99 * (m_maxWave - m_minWave) /
            (m_maxWave - m_plotterWidget->bands().first()));
        ui->horizontalSliderMaxBand->setValue(
            99 * (m_maxWave - m_minWave) /
...
WavesRangeDialog::~WavesRangeDialog() { delete ui; }

void WavesRangeDialog::setupGui(QVector<double> &waves, QVector<double> &values,
...
void WavesRangeDialog::drawMinLine() {
    m_minVerticalLine->point1->setCoords(
        m_minWave, m_plotterWidget->customPlot()->yAxis->range().minRange);
    m_minVerticalLine->point2->setCoords(
        m_minWave, m_plotterWidget->customPlot()->yAxis->range().maxRange);
    m_plotterWidget->customPlot()->replot();
}

void WavesRangeDialog::drawMaxLine() {
    m_maxVerticalLine->point1->setCoords(
        m_maxWave, m_plotterWidget->customPlot()->yAxis->range().minRange);
    m_maxVerticalLine->point2->setCoords(
        m_maxWave, m_plotterWidget->customPlot()->yAxis->range().maxRange);
    m_plotterWidget->customPlot()->replot();
}

void WavesRangeDialog::setMaxWave(double newMaxWave) { m_maxWave = newMaxWave; }

double WavesRangeDialog::minWave() const { return m_minWave; }

void WavesRangeDialog::setMinWave(double newMinWave) { m_minWave = newMinWave; }

double WavesRangeDialog::maxWave() const { return m_maxWave; }

void WavesRangeDialog::on_horizontalSliderMinBand_sliderMoved(int position) {
    double percent = static_cast<double>(position) / 99;
    m_minWave =
        m_maxWave - (m_maxWave - m_plotterWidget->bands().first()) * percent;
    ui->doubleSpinBoxStartWave->setValue(m_minWave);
    drawMinLine();
}
...
void WavesRangeDialog::on_horizontalSliderMaxBand_sliderMoved(int position) {
    double percent = static_cast<double>(position) / 99;
    m_maxWave =
        m_minWave + (m_plotterWidget->bands().last() - m_minWave) * percent;
    ui->doubleSpinBoxEndWave->setValue(m_maxWave);
    drawMaxLine();
}
...
void WavesRangeDialog::keyPressEvent(QKeyEvent *event) {
    switch (event->key()) {
        case Qt::Key_Enter:
        case Qt::Key_Return:
            if (ui->doubleSpinBoxEndWave->hasFocus() ||
                ui->doubleSpinBoxStartWave->hasFocus()) {
                ui->widgetSpectra->setFocus();
            } else {
                this->accept();
            }
