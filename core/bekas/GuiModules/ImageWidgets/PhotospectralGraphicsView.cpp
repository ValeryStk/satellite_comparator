...
void PhotospectralGraphicsView::setShowingImage(
    QString imageFilePath, QVector<QPair<int, int>> spFov) {
    m_scene->clear();
...
void PhotospectralGraphicsView::paintImageAndFov() {
    if (!m_imageFilepath.isEmpty()) {
        QImage fullImage = QImage(m_imageFilepath, "PNG");
        QImage image =
            fullImage.scaled(QSize(this->width(), this->height()),
        ...
        for (int i = 0; i < m_specFov.count(); i++) {
            QPair<int, int> currentPoint =
                recalcPointCoords(m_specFov.at(i), fullImage.size());
        ...
        m_scene->addSimpleText("Изображение отсутствует",
                               QFont("Helvetica", 16));
    }
}

QPolygonF PhotospectralGraphicsView::getPolygonFromPoints(
    QVector<QPair<int, int>> fovPoints) {
...
QPair<int, int> PhotospectralGraphicsView::recalcPointCoords(
    QPair<int, int> baseCoords, QSize baseSize) {
    QPair<int, int> scaledPoint;
    QSize currentSize = QSize(this->width(), this->height());
    scaledPoint.first = static_cast<double>(this->width()) * baseCoords.first /
                        baseSize.width();
    scaledPoint.second = static_cast<double>(this->height()) *
                         baseCoords.second / baseSize.height();
    return scaledPoint;
}
