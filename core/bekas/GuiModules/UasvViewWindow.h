...
public:
    ...
    explicit CustomStringListModel(const QStringList &strings,
...
private:
    ...
    /**
     * @brief parseInputFolder  Function to find all the spectra in start
     * directory
     */
    ...
    void drawSpectrum(QVector<double> &waves, QVector<double> &values,
                      double maxValue, db_json::BandUnits bandsUnits,
                      db_json::SpectrumUnits spUnits, QString spName);

    void fillMetaData(db_json::META_DATA spectrumMd);

    QString getSpecFileNameSuffix(int &currSpecIndex);

    void checkClassifAndSetupCBox(QString spClassification,
                                  QComboBox *settingCBox, QString textForEmpty);
    ...
    CustomStringListModel
        *m_slModel;  //!< String list model (for the list of spectra)
    UdpJsonRpc *m_rpc;
